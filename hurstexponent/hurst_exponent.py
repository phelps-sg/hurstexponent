...
def standard_hurst(
    ...
    if fitting_method not in ["mle", "least_squares", "OLS"]:
        raise ValueError(
            f"Unknown method: {fitting_method}. Expected 'mle' or 'least_squares' 'OLS'."
        )
    ...
    valid_lags_and_values = [
        (lag_size, std_of_sums(series, lag_size))
        for lag_size in lag_sizes
        if std_of_sums(series, lag_size) is not None
        and std_of_sums(series, lag_size) != 0
    ]
    ...
    if fitting_method == "mle":
        ...
        initial_guess = [0.5, 0]
        _residuals = lambda params, N, y_values: y_values - _hurst_function(
            N, params[0], params[1]
        )
        result = least_squares(
            _residuals, initial_guess, args=(valid_lags, y_values), bounds=bounds
        )
        H, c = result.x
...
def generalized_hurst(
    ...
    S_q_tau_values, valid_lags = zip(
        *[
            (structure_function(series, moment, lag), lag)
    ...
    if fitting_method == "mle":
        ...
        result = minimize(
            ...
            args=(
                np.array(valid_lags),
                np.array(S_q_tau_values),
                _generalized_function,
            ),
            bounds=bounds,
        )
        H_q, c = result.x
        ...
        bounds = (lower_bounds, upper_bounds)
        _residuals = (
            lambda params, lag, S_q_tau_values: S_q_tau_values
            - _generalized_function(lag, params[0], params[1])
        )
        result = least_squares(
            _residuals, initial_guess, args=(valid_lags, S_q_tau_values), bounds=bounds
        )
        H_q, c = result.x
    else:  # Log-log OLS regression fitting method
...
if __name__ == "__main__":
    ...
    series = simple_series(
        length=99999, volatility=0.0002, seed=70
    )  # avg. daily market volatility
    ...
    print(
        f"Hurst Estimate via Standard Hurst: {H}, D constant: {D if D is not None else 'N/A'}, ({interpretation})"
    )

    # Generalized Hurst
    H, c, data, interpretation = generalized_hurst(series)
    print(
        f"Hurst Estimate via Generalized Hurst: {H}, D constant: {D if D is not None else 'N/A'}, ({interpretation})"
    )
    ...
    print(
        f"Hurst Estimate via R/S: {H}, c constant: {c if c is not None else 'N/A'}, ({interpret_hurst(H)})"
    )
    ...
    axs[0].plot(
        lag_sizes,
        D * np.array(lag_sizes) ** H,
        "g--",
        label=f" Standard Hurst (H={H:.2f})",
    )
    axs[0].loglog()
    ...
    c = np.mean(np.log10(S_q_tau)) - H * np.mean(
        log_tau
    )  # calculate constant with log_S_q_tau
    axs[1].plot(tau, S_q_tau, "b.", label="Observed Values")  # plot with raw values
    axs[1].plot(
        tau,
        10 ** (c + np.array(log_tau) * H),
        "g--",
        label=f"Generalized Hurst (H={H:.2f})",
    )  # plot using line equation with base 10 exponent
    axs[1].loglog()
    ...
    def bootstrap(
        estimator: Callable[[Any], Tuple[float, float, List[List[float]]]],
        reps: int,
        seed: int,
    ) -> np.array:
        np.random.seed(seed)
